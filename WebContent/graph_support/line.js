...
// Get the data
d3.json("../feeds/total_count_weekly.jsp", function(data) {
	...
	var myObserver = new ResizeObserver(entries => {
		entries.forEach(entry => {
			...
			if (newWidth > 0) {
				d3.select("#line-wrapper").select("svg").remove();
				maxWidth = newWidth;
				draw();
			}
		});
	});
	myObserver.observe(d3.select("#line-wrapper").node());
	...
	function draw() {
		...
		var svg = d3.select("#line-wrapper").append("svg")
